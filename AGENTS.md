...
3. Read the canonical node in `okf/<hierarchy_id>.md` before citing or reasoning about a specific process element.
...
- Cite relevant nodes as `okf/<hierarchy_id>.md` and include the `pcf_id` in the answer.
...
- A node filename equals its `hierarchy_id` plus `.md`.
